...
public abstract class Window {
	...
	private boolean isSelected = false, isOver = false;
	protected boolean shift = false;
	protected final Pacman app;
	protected float mouseX, mouseY;

	public Window(Pacman app, float x, float y, float w, float h, String windowName) {
		this.x = x;
		this.y = y;
		this.w = w;
		this.h = h;
	...
	public Window(Pacman app, float x, float y, float w, float h) {
	...
	private void update() {
		...
		if (!isHidden) {
			...
			if (!isMoving && isVisible && (mouseX > x && mouseX < x + w && mouseY > y && mouseY < y + h + 20)) {
				isOver = true;
			...
			if (!isMoving && isVisible && (mouseX > x && mouseX < x + w && mouseY > y + h && mouseY < y + h + 20)) {
				isOver = true;
			} else if (!isMoving && isVisible) {
				isOver = false;
			}
